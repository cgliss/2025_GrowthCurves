function [timeFine,od,conc, drugHalfAUC, noDrugGompertz, predictedParams] = GC_matching_halfauc_Gompertz_sf(allData,drugLabel,tfDEBUG,dilutionFactor,AUCfactor, Strain,timeResolution, timeLimited, gompertz_model, startColor, A600)
...
for j5 = 1:length(aucFrac)
    ...
    legs = [legs, aucFrac(j5)]; 
    catch
        % if normAUCatConc doesn't reach aucFrac
        continue
    end 
end
% removing unassigned 
...
text(0.5, 0.7, sprintf('normalized AUC = %g - %g',min(legs), max(legs) ))
%plot(timeFine(1:ind), exp(fitOD(halfAUCidx, 1:ind))*A600, 'r','LineWidth',2)
%legend(string(legs), 'Location','eastoutside', 'IconColumnWidth',5)
